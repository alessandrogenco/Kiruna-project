...
describe("POST /api/upload", () => {
  test("Successfully uploads a file", async () => {
    ...
    const description = "Sample PDF File";
    const lastIDs = [123];
    ...
    const response = await request(app)
      .post(`${baseURL}upload?documentId=${documentId}&resourceType=${resourceType}&description=${description}`)


      expect(response.status).toBe(201);
      expect(response.body).toEqual({
        message: 'Files uploaded successfully',
        resourcesIds: lastIDs.map(id => ({ resourceId: id }))
      });
      expect(FileUploadDao.prototype.addOriginalResources).toHaveBeenCalledWith(documentId, expect.any(Array));
  
      // Rimuovere il file di esempio dopo il test
